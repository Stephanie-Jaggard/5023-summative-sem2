...
resi_plot <- augbutterfly_ls1 %>% 
  ...
  labs(x = "Temperature (Celsius)",
       y = "Fitted model value for forewing (Millimeters)")+
  theme_classic()
...
broom::augment(butterfly_ls1, newdata=tibble(jun_mean=c(11,17)), interval="confidence")
#11 deg = 13 mm 
#17 deg = 14.2 mm
...
pred_table <- butterfly_ls1 %>% 
  broom::augment(butterfly_ls1, newdata=tibble(jun_mean=c(11,17)), interval="confidence") %>%
  kbl(caption = "Table 1 - Predictions for the size of Silver Spotted Skipper butterfly forewings in new temperature environments",
      col.names = c("Temperature (Celsius)", "Predicted forewing size (Millimeters)", "Lower Confidence Interval","Upper Confidence Interval")) %>% 
...
male_resi_plot <- male_augbutterfly_ls1 %>% 
  ...
  geom_segment(aes(x=jun_mean, 
               ...
               linetype="longdash", colour="darkgrey")+
  labs(x = "Temperature (Celsius)",
       y = "Fitted model value for male forewing (Millimeters)")+
  theme_classic()
...
male_pred_table <- male_butterfly_ls1 %>% 
  broom::augment(male_butterfly_ls1, newdata=tibble(jun_mean=c(11,17)), interval="confidence") %>%
  kbl(caption = "Table 1 - Predictions for the size of Silver Spotted Skipper butterfly forewings in new temperature environments",
      col.names = c("Temperature (Celsius)", "Predicted male forewing size (Millimeters)", "Lower Confidence Interval","Upper Confidence Interval")) %>% 
  kable_styling(bootstrap_options = "striped", full_width = F, position = "left")
